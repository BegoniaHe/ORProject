...
function test_deterministic_cost_minimization_results()
    ...
    for y in 2020:2025, local_pricing in [true, false]
        @test isa(
        ...
        @test isa(
            result_data[y, local_pricing]["flow"], JuMP.Containers.DenseAxisArray{Float64,3}
        )
        @test isa(
...
function test_deterministic_welfare_maximization_results()
    ...
    for y in 2020:2025, local_pricing in [true, false]
        ...
        @test isa(
            result_data[y, local_pricing]["flow"], JuMP.Containers.DenseAxisArray{Float64,4}
        )
        @test isa(
            result_data[y, local_pricing]["extraction_capacity"],
            JuMP.Containers.DenseAxisArray{Float64,2},
...
function test_stochastic_cost_minimization_results()
    ...
    for (idx, y) in enumerate(2020:2025)
        ...
        @test isa(
            result_data[idx][2][:injection], JuMP.Containers.DenseAxisArray{Float64,3}
        )
        @test isa(
            result_data[idx][2][:extraction], JuMP.Containers.DenseAxisArray{Float64,3}
        )
        @test isa(
        ...
        @test isa(
            result_data[idx][2][:generation], JuMP.Containers.DenseAxisArray{Float64,3}
        )
